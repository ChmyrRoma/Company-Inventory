export const getProductDeclension = (count: number, language: 'ua' | 'ru' | 'en' = 'ua') => {
    const lastDigit = count % 10;
    const lastTwoDigits = count % 100;

    if (language === 'ua') {
        if (lastTwoDigits >= 11 && lastTwoDigits <= 14) return 'Продуктів';
        if (lastDigit === 1) return 'Продукт';
        if (lastDigit >= 2 && lastDigit <= 4) return 'Продукти';
        return 'Продуктів';
    }
